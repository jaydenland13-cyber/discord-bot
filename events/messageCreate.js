...
async function handleTicketAI(message) {
  ...
  let pingText = '';
  if (wantsStaff(message.content) && !ticket.staffPinged && config.supportRoleId) {
    pingText = `<@&${config.supportRoleId}> — this user is asking for staff.\n\n`;
    ticket.staffPinged = true;
  }

  saveTicket(message.channel.id, ticket);

  await message.reply({ content: (pingText + answer).slice(0, 2000) });
}
